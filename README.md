...
```Шаг 2.``` После запуска скрипта вы увидите список комманд.

```./createDataTable``` - следует прописать в самом начале работы. Создаст все необходимые для работы таблицы в базе данных.  

```./addData``` - активирует функцию добавления данных об аккаунтах в таблицу. Необходимые данные: номер телефона, api id,  
api hash. Последние два нужно создать на сайте https://my.telegram.org/auth?to=apps во вкладке API.   

```./settings``` - перекинет в менюшку настроек. Там вы увидите следующие комманды:

```./addRefCode``` - добавляет реферальный
./updRefCode
./addBlockIoApiKey
./updBlockIoApiKey
./back
...
```./startFarming``` - запускает фарм. После запуска фарма остановка скрипта будет возможна следующим способом: Ctrl + C  
Однако, после этого нужно будет перезапустить скрипт, как указано в первом шаге.

```./checkBalance``` - проверяет баланс в боте на аккаунтах, выводим общий баланс.

```./withdrawBalance``` - выводит баланс в боте на кошельки с сайта https://block.io/.
